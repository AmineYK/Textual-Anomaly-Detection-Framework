...
#         self.n_ref = n_ref
#         # Pré-calcule les stats sur CPU, on les déplacera dans forward
#         ref = torch.normal(mean=0., std=torch.ones(n_ref))
#         self.mu_ref = ref.mean().item()      # scalaire Python → pas de problème device
...
#     #     # hidden_state : [batch, seq_len, hidden]  (plus de passage par sbert)
#     #     t = torch.tanh(self.W1(hidden_state))
#     #     t = F.softmax(self.W2(t), dim=1)          # [batch, seq_len, num_heads]
#     #     attention_matrix = t.transpose(1, 2)       # [batch, num_heads, seq_len]
#     #     outputs = attention_matrix @ hidden_state  # [batch, num_heads, hidden]
#     #     outputs = torch.flatten(outputs, start_dim=1)
        
#     #     topk = max(int(outputs.size(1) * self.top_k), 1)
#     #     outputs = torch.topk(outputs, topk, dim=1)[0]  # plus de abs()
#     #     return outputs.mean(dim=1), attention_matrix
...
#         outputs = attention_matrix @ hidden_state   # [batch, heads, hidden]
#         outputs = torch.flatten(outputs, start_dim=1)  # [batch, features]

#         # 🔥 TOP-K AVANT réduction
#         topk = max(int(outputs.size(1) * self.top_k), 1)
#         outputs = torch.topk(torch.abs(outputs), topk, dim=1)[0]

#         # 🔥 puis aggregation
#         outputs = outputs.mean(dim=1)  # [batch]
...
#         def get_token_embeddings(tokenized):
#             input_ids = tokenized['input_ids'].to(self.device)
#             attention_mask = tokenized['attention_mask'].to(self.device)
#             all_embeddings = []
#             # Par batch pour éviter les OOM sur de grands corpus
#             bs = 64
#             with torch.no_grad():
...
#                     }
#                     out = self.model.sbert(features)
#                     all_embeddings.append(out['token_embeddings'].cpu())
#             return torch.cat(all_embeddings, dim=0)  # [N, seq_len, hidden]

#         # Gèle SBERT — seuls W1 et W2 seront entraînés
#         for param in self.model.sbert.parameters():
#             param.requires_grad = False

#         # Tokenise tout
#         tok_inlier = tokenize(inliers)
#         tok_anom   = tokenize(anomalies)
#         tok_test_in = tokenize(test_inliers)
#         tok_test_an = tokenize(test_anom)

#         # Calcule les embeddings une seule fois
#         emb_inlier   = get_token_embeddings(tok_inlier)   # [N_in, seq, hid]
#         emb_anom     = get_token_embeddings(tok_anom)     # [N_an, seq, hid]
#         emb_test_in  = get_token_embeddings(tok_test_in)
#         emb_test_an  = get_token_embeddings(tok_test_an)

#         # Labels et indices pour le sampler
...
#             torch.cat([emb_inlier, emb_anom]),   # embeddings directs, plus d'input_ids
#             labels_train,
#             normal_idx,
#             outlier_idx
#         )
#         test_inlier_dataset = CustomDataset(emb_test_in,  torch.zeros(len(test_inliers)))
#         test_anom_dataset   = CustomDataset(emb_test_an,  torch.ones(len(test_anom)))
...
#                 outputs, A = self.model(embeddings)
...
#                 embeddings = embeddings.to(self.device)   # ← même correction
...
#         auc, fpr95, ap  = ev.evaluation(gt, preds, verbose=False)
...
class FATETrainer:
    def __init__(self,
        ...
        # Model + tokenizer
        f_name = f"sentence-transformers/{model_name}"
        self.tokenizer = AutoTokenizer.from_pretrained(f_name)
        self.model = SBERTWithAttention(model_name=self.model_name, top_k=top_k).to(self.device)
