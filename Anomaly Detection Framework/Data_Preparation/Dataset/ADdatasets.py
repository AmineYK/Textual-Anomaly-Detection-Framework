...
##### REPENSE A LA METHODE concatenate_datasets from datasets ######
...
class DatasetWrapper(Dataset):
    def __init__(self, dataset, type_emb=None):

        self.type_emb = type_emb
        self.dataset = dataset
        self.texts = self.dataset['text']
        self.labels = self.dataset['anomaly_class']
        if type_emb is not None : self.inputs = self.dataset[EmbeddingMapping[type_emb]]
    ...
    def __getitem__(self, idx):
        ...
        if self.type_emb is not None : inputs = self.inputs[idx]


        if self.type_emb is not None : return torch.tensor(inputs), torch.tensor(label), text
        else: return torch.tensor(label), text
...
#         return inputs, torch.tensor(label, dtype=torch.long), text, idx
...
class CVDDDatasetWrapper(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.embedding_type in ['glove', 'fasttext']:
            ...
            if self.vocab is None:
                raise Exception(f"{self.embedding_type} encoding requires the parameter vocab")
            ...
            if self.tokenizer is None:
                raise Exception(f"{self.embedding_type} encoding requires the parameter tokenizer")
            ...
            inputs = torch.tensor(
                self.dataset['tfidf_embedding'][idx],
                dtype=torch.float32
            )
