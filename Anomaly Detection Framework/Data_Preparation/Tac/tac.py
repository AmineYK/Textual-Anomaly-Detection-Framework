...
def replace_inliers_with_anomalies(inlier_dataset, anomaly_dataset, anomaly_rate):

    N = inlier_dataset.num_rows
    n_replace = int(N * anomaly_rate)

    # 1) sélectionner les inliers à garder (N - n_replace)
    keep_indices = np.random.choice(N, N - n_replace, replace=False)
    inlier_kept = inlier_dataset.select(keep_indices)

    # 2) sélectionner n_replace anomalies
    anomaly_indices = np.random.choice(anomaly_dataset.num_rows, n_replace, replace=False)
...
def textual_anomaly_contamination_reuters(dataset, inlier_topic, type_tac='ruff', anomaly_rate=0.1, is_trainset=True):
    ...
    if type_tac == 'ruff':
        ...
        if is_trainset:
            # ancienne logique pour train — inchangée
            n_anomalies = int((anomaly_rate * inlier_dataset_ruff.num_rows) / (1 - anomaly_rate))
            anomaly_indices = np.random.randint(0, anomaly_dataset_ruff.num_rows, n_anomalies)
            anomaly_dataset_ruff = anomaly_dataset_ruff.select(anomaly_indices)
            ...
            return inlier_dataset_ruff, anomaly_dataset_ruff
        
        else:
            # nouvelle logique : remplacer les inliers retirés par anomalies
            return replace_inliers_with_anomalies(inlier_dataset_ruff, anomaly_dataset_ruff, anomaly_rate)
    ...
    if type_tac == 'pantin':
        ...
        if inlier_topic not in parent_topics.keys():
            raise Exception(" Warning ! the inlier topic requested doesn't exist !")           
            
        inlier_dataset_pantin = dataset_pantin.filter(lambda x: x['parent_topic'] == inlier_topic)
        anomaly_dataset_pantin = dataset_pantin.filter(lambda x: x['parent_topic'] != inlier_topic)
            
        if is_trainset:
            # ancienne logique train
            n_anomalies = int((anomaly_rate * inlier_dataset_pantin.num_rows) / (1 - anomaly_rate))
            anomaly_indices = np.random.randint(0, anomaly_dataset_pantin.num_rows, n_anomalies)
            anomaly_dataset_pantin = anomaly_dataset_pantin.select(anomaly_indices)
            ...
            return inlier_dataset_pantin, anomaly_dataset_pantin
        
        else:
            return replace_inliers_with_anomalies(inlier_dataset_pantin, anomaly_dataset_pantin, anomaly_rate)
...
def textual_anomaly_contamination_20newsgroups(dataset, inlier_topic, type_tac='ruff', anomaly_rate=0.1, is_trainset=True):
    ...
    if inlier_topic not in groups:
        raise Exception(" Inlier topic invalid ")
